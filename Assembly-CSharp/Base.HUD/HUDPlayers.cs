...
public class HUDPlayers
{
	...
	public static void usedBan(SleekFrame frame)
	{
		...
		{
			NetworkTools.ban(
				NetworkUserList.users[HUDPlayers.selected].player, 
				NetworkUserList.users[HUDPlayers.selected].name, 
				NetworkUserList.users[HUDPlayers.selected].id, 
				string.Concat("Steam ID banning ", NetworkUserList.users[HUDPlayers.selected].name, "."),
				PlayerSettings.id
			);
	...
	public static void usedBanned(SleekFrame frame) {
		...
		if (offsetY < NetworkBans.GetBannedPlayers().Count)
		{
