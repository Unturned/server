...
public class HUDDedicated : MonoBehaviour
{
	...
	public static void usedBan(SleekFrame frame) {
		NetworkTools.ban(
			NetworkUserList.users[HUDDedicated.selected].player,
			NetworkUserList.users[HUDDedicated.selected].name,
			NetworkUserList.users[HUDDedicated.selected].id,
			string.Concat("Steam ID banning ", NetworkUserList.users[HUDDedicated.selected].name, "."),
			PlayerSettings.id
		);
	}
